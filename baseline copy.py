...
class KeyboardPlayerPyGame(Player):
    def __init__(self):
        ...
        self.K = np.array([[cam_fx, 0, cam_cx],
              [0, cam_fy, cam_cy],
              [0, 0, 1]])

        # 畸变系数
        self.dist_coeffs = np.array([cam_k1, cam_k2, cam_p1, cam_p2, cam_k3])
    ...
    def compute_rotation_angle(self, image1, image2):
        # 将图像转换为灰度图像
        undist_image1 = cv2.undistort(image1, self.K, self.dist_coeffs)
        undist_image2 = cv2.undistort(image2, self.K, self.dist_coeffs)

        gray1 = cv2.cvtColor(undist_image1, cv2.COLOR_BGR2GRAY)
        gray2 = cv2.cvtColor(undist_image2, cv2.COLOR_BGR2GRAY)


        # 使用ORB特征检测器检测特征点和计算描述子
        detectors = [cv2.AKAZE_create(), cv2.SIFT_create(), cv2.ORB_create()]
        matchers = [cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True), cv2.FlannBasedMatcher()]
        kdPair = []
        degrees = []
        for detector in detectors:
            keypoints1, descriptors1 = detector.detectAndCompute(gray1, None)
            keypoints2, descriptors2 = detector.detectAndCompute(gray2, None)
            kdPair.append(((keypoints1, descriptors1), (keypoints2, descriptors2)))
        
        for matcher in matchers:
            for ((keypoints1, descriptors1), (keypoints2, descriptors2)) in kdPair:
            ...
            if event.type == pygame.KEYUP:
                ...
                match self.eventType:
                    ...
                    case pygame.K_LEFT:
                        ...
                        plt.title('Image 2')
                        plt.show()
                        angle = self.compute_rotation_angle(image1, image2)
                        print(angle)
                    case pygame.K_RIGHT:
                        image1 = self.fpv 
                        image2 = self.prevFPV
