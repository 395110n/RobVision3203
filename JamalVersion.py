...
class KeyboardPlayerPyGame(Player):
    ...
    def see(self, fpv):
        ...
        # plt.imshow(self.fpv)
        # plt.show()
        fpv_process = fpv.copy()
        if fpv is None:
            return

        if len(fpv_process.shape) == 3:
            fpv_process = cv2.cvtColor(fpv_process, cv2.COLOR_BGR2GRAY)

        if fpv_process.shape[0] != self.vo_state['cam']['height'] or fpv_process.shape[1] != self.vo_state['cam']['width']:
            fpv_process = cv2.resize(fpv_process, (self.vo_state['cam']['width'], self.vo_state['cam']['height']))
        ...
        if self._state:
            if self._state[1] == Phase.EXPLORATION:
                ...
                cv2.imwrite(save_path, fpv_process)
                VLAD = self.get_VLAD(fpv_process)
        ...
        if self.last_act != Action.IDLE:
            self.vo_state = self.frame_update_step(self.vo_state, fpv_process, kMinNumFeature, STAGE_DEFAULT_FRAME, STAGE_SECOND_FRAME, STAGE_FIRST_FRAME)
